...
const
    ...
    isMobile = parameters.os === 'webshadowmobile' || ((window.self === window.top) ? isMobileMediaQuery.matches : (/(?!.*ipad)^.*(iphone|ipod|((?:android)?.*?mobile)|blackberry|nokia)/i).test(userAgent) || ((/android/i).test(userAgent) && isMobileMediaQuery.matches)),
